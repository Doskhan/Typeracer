...
class Admin(QWidget):
	...
	def buttonEditClicked(self):
		...
		surname = self.le[id][2].text()
		date = self.le[id][3].text()
		password = self.le[id][4].text()
		c.execute("UPDATE qwerty SET name=\'"+ name +"\', surname=\'" + surname +"\', email=\'" + date +"\' , password=\'" + password +"\' WHERE id=\'" + str(id) + "\'")
		self.conn.commit()
	...
	def buttonDeleteClicked(self):
		...
		sender = self.sender()
		id = int(sender.objectName())
		name = self.le[id][3].text()
		c.execute("DELETE FROM qwerty WHERE email='%s'" %name)
		self.conn.commit()
	...
	def buttonInsertClicked(self):
		...
		surname = self.le[id][2].text()
		date = self.le[id][3].text()
		date = self.le[id][4].text()
		if name!="" and surname!="" and date!="":
			a.execute("INSERT INTO qwerty (name, surname, email,password) VALUES(\'" + name +"\', \'" + surname +"\', \'" + email +"\',\'" + password +"\')")
		c.execute("SELECT * FROM qwerty")
